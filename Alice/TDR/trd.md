...
staleness: fresh
known_verify_flags: ["§3.2 Xe absorption-length reference at 5 keV", "§3.2 operational Lorentz angle at B = 0.5 T", "§3.2 Run 1/2 as-operated gas gain", "§5.2 as-measured Run 1/2 pion rejection"]
...
- **Dense interfaces for yield:** emission from successive interfaces adds coherently within the **formation zone** ζ ≈ γ λ / (2π), which at γ = 2000 and λ ~ 0.2 nm (5 keV photon) is of order hundreds of micrometres. Interfaces closer than ζ interfere; further apart than ζ they add incoherently.
...
- ADC clock runs at 10 MHz (100 ns intervals).
...
- **Supermodule** — one of 18 azimuthal TRD modules spanning 20° in φ and the full 6-layer radial depth (half the z range); the detector has 18 × 2 = 36 per-side? **No** — 18 supermodules total, each spanning full z; see §3.1.
...
- [ ] **Arithmetic closure** (machine-checkable):
  - §3.3 pad-count: `144 × 74 (mean rows) × 6 × 18 = ~1,150,848` — consistent with 1,156,032 stated value within rounding on mean row count.
  ...
  - §5.1 TRD lever arm extension: `3.68 − 2.50 = 1.18 m` vs TPC-only `2.466 − 0.848 = 1.618 m`; fractional increase = `(3.68 − 2.50) / (2.50 − 0.848) × ...` — spec says +48 %; check on actual boundary values if clarified in Turn 2.
...
|---|---|---|
| [`./tpc.md`](./tpc.md) | TL;DR, §1, §4.1, §4.4, §5.1, §5.2, §5.5, §5.6 | live (wiki-v1, APPROVED WITH COMMENTS) |
| [`./its.md`](./its.md) | TL;DR, §5.1, §5.5, §5.6 | live (DRAFT, wiki-v0) |
...
*End of TRD Source of Truth, wiki-v0 (derived from source v0.3 turn-1, 2026-04-19). This page is explicitly `DRAFT` and marked "source-partial" — promotion past DRAFT should wait for source Turn 2 (performance table + operational history), not just for a reviewer pass. Next reviewer pass should confirm URL reachability for CDS 519145 + file cer-2275567.pdf, resolve the §10.1 flags with the TRD performance paper, and flag this wiki page for re-indexing once source Turn 2 arrives.*
