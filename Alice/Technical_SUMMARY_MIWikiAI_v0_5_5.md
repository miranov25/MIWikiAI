...
project: MIWikiAI
version: v0.5.5 (2026-05-05)
status: DRAFT — paired update consolidating MIWikiAI_QRC v0.5.3 + v0.5.4 + v0.5.5 evolution; awaits architect ratification
quick_reference: ./MIWikiAI_Quick_Reference_Card_v0_5_5.md
prior_version: v0.5.2 (2026-05-02) — intermediate v0.5.3 and v0.5.4 had no Tech_SUMMARY bumps; v0.5.5 consolidates
supersedes: v0.5.2 by consolidating three QRC bumps: v0.5.3 (dispatch-discipline), v0.5.4 (section-ownership review), v0.5.5 (Coder-side discipline). Three new Phase 0.2 cycles documented (cycle-2/3/4 of Common_utilities_API.md). Prefilter pipeline v1.0→v1.3 added to §4.
date: 2026-05-05
revision_note: "v0.5.5 consolidates three intermediate QRC bumps. Three new Phase 0.2 cycles (2/3/4) documented. NEW §4b prefilter pipeline section (prepare_review.py v1.0→v1.3 with mistletoe AST + char-exact VERBATIM diff + Signal-block field-presence). §6 roster revised: Geminis replaced with GPTs after cycle-3+4 empirical evidence (6 Gemini hallucinated P0s, 0 unique findings). §8 items 25-29 added (dispatch-discipline, smoke test, aspect taxonomy, prefilter ratchet, Coder local-convergence + architect-data)."
...
architect_ratifications_2026_05_03:
  - "QRC v0.5.3 ratified — dispatch-document discipline §§2.7+4.5+4.6+4.7"
  - "QRC v0.5.4 ratified — section-ownership review §§4.7-4.10 (2+2+1 deep coverage, sprint cap=5)"
architect_ratifications_2026_05_05:
  - "QRC v0.5.5 ratified per 3-Opus approval synthesis (Claude1+Claude2+Claude10 unanimous APPROVE-AS-IS)"
  - "QRC v0.5.6 ratified evening — 8 non-blocking refinements consolidated"
  - "Reviewer pool composition revised: 6 Sonnet + 3 GPT + 3 Opus (down from 6+6+3, Geminis empirically zero-signal across cycles 3-4)"
  - "Common_utilities_API.md v0.5 produced incorporating cycle-4 15 CONV findings"
  - "Prefilter v1.3 deployed — closes 4 coverage gaps (bare-name fabrication, char-exact VERBATIM diff, label discipline, Signal field completeness)"
---
...
### 4b. Prefilter pipeline (NEW v0.5.5, see `prepare_review.py`)

Python module + bash wrapper running four mechanical pre-flight checks before each `_API.md` dispatch. Origin: cycle-3+cycle-4 evidence that mechanical checks across 14+ reviewers per cycle produced redundant work and missed structural defects. Prefilter discharges mechanical work at global scale; reviewer attention redirects to judgment-aspect work (per QRC v0.5.5 §4.11 ratchet rule).

**Architecture:** AST-based markdown parser (mistletoe). NO regex against raw markdown structure (v0.x → v1.0 migration; root cause of 6+ false-PASS bugs in v0.x).

**Four checks:**

| Check | What it verifies | Tier |
|---|---|---|
| `anchor_check` | Every `[label](#anchor)` link resolves to a heading slug | T1 |
| `verbatim_check` | Every `[VERBATIM <path>:L<a>-L<b>]` block: file exists, range valid, **content character-exact match against source** (v1.3 added char-exact diff) | T2 |
| `counter_check` | Every Signal block: all 6 mandated fields present (QRC §2.6); numeric values match `usage.csv` | T2 |
| `prose_fabrication_check` | Known fabricated identifiers (`kCCDBPRIO\|kRTF\|kEXIM\|CCDBPRIO\|RTF\|EXIM` per cycles 2+4) absent from main body; front-matter occurrences classified as disclosure (PASS) | T1 |

**Bundle assembly side-effect:** prefilter auto-detects source subtree from artifact's `upstream:` front-matter and bundles entire detected subtree (e.g., `Common/Utils/` for ConfigurableParam family). v1.2 closed cycle-3 false-narrow-source-zip bug; v1.3 added all 4 check enhancements.

**Empirical history:**

| Cycle | Prefilter version | Coverage gaps surfaced | Closed in |
|---|---|---|---|
| 3 | v0.x (regex) | 6+ false-PASS classes (heading regex, Signal-block detection, multi-file basename, GPU macro, std-namespace) | v1.0 (AST migration) |
| 4 | v1.2 (AST) | 4 coverage gaps (bare-name fabrication, char-exact diff, informal labels, incomplete Signal blocks) | v1.3 |
| 5+ | v1.3 | TBD per cycle-5 confusion-matrix data | v1.4 backlog |

**Coverage classes (v1.3):** T1 string-grep checks (prose_fabrication); T2 structural checks (anchor, verbatim, counter); T3 semantic checks NOT in prefilter (judgment-aspect reviewer work per QRC §4.7 Aspects C, E, F).

> *Origin: cycle-3 false-PASS pattern → cycle-4 panel synthesis §3 → v1.3 closes all 4 gaps. Codifies trust ratchet that QRC v0.5.5 §4.11 governs.*
...
**Reviewer pool revision (NEW v0.5.5, 2026-05-05):** Architect ratified replacement of all 6 Gemini reviewers with 3 GPT reviewers based on cycle-3 + cycle-4 empirical evidence (6 Gemini hallucinated P0s, 0 unique source-verified findings across 2 cycles). Active pool: **Sonnet1-Sonnet6, GPT1-GPT3, Opus1-Opus3 = 12 reviewers** (down from 15). §4.8 2+2+1 section coverage adjusted to 2 Sonnet + 1 GPT + 1 Opus per section pending cycle-5 data on GPT signal quality. Reduction reactive to evidence, not budget-driven.
...
- 23. **Reviewer source-access protocol** (NEW v0.5.2, P0 fabrication / P1 unverified-finding). Symmetric to Coder anti-silent-paraphrase rule (§20). If a reviewer lacks source, the reviewer requests it. Producing a confident finding without verifying cited source is P1; inventing or guessing is P0. Codified in MIWikiAI_QRC v0.5.2 §3.5 after cycle-1 contrast: Sonnet3 caught real bugs by red-team source-fetching, Gemini2 produced a hallucinated anchor finding without source access.
- 24. **Standardized review-bundle requirement** (NEW v0.5.2). Every `_API.md` cycle-2+ dispatch ships a `<artifact>_v<version>_review_bundle.zip` with MANIFEST + artifact + cycle-0 self-review + prior consolidated summary + reviewer prompts + cited source files + counter outputs + governance docs. Standardizes evidence distribution; prevents spurious reviewer divergence from evidence asymmetry. Coder produces; architect verifies before dispatch. Codified in MIWikiAI_QRC v0.5.2 §4.4. Cycle-3+ on `_API.md` artifacts is first compliance window (cycle-2 of `Common_utilities_API.md` v0.2 predates this rule).


**Items added v0.5.3-v0.5.5:**

25. **Dispatch-document discipline (v0.5.3 §4.5):** dispatch documents must follow 10-item required-contents list; action-first anti-pattern banned. Origin: cycle-3 first-attempt dispatch.

26. **Pre-dispatch smoke test (v0.5.3 §4.6):** cycles ≥3 require single-reviewer smoke test before full panel sees the artifact.

27. **Six-aspect review taxonomy (v0.5.3 §4.7):** Aspects A-F with Opus-budget allocation. v0.5.4 reframed to "three-dimensional review assignment" (reviewer × document_section × aspect_focus) with 2+2+1 deep section coverage.

28. **Prefilter ratchet (v0.5.5 §4.11):** mechanical checks discharged at global level by `prepare_review.py`; reviewers redirect attention to judgment-aspect work; backslide-protection rollback rule for false-PASS/false-FAIL.

29. **Coder local-convergence + architect-data-not-mocks (v0.5.5 §§4.12+4.16):** Coder runs all validation locally before architect notification using architect's most-recent uploaded source data. Origin: cycle-4 v0.5 → v0.5.1 incident.
...
- **v0.5.5 (2026-05-05).** Paired update with `MIWikiAI_Quick_Reference_Card_v0_5_5.md` consolidating three QRC bumps since v0.5.2: v0.5.3 (dispatch-document discipline), v0.5.4 (section-ownership review), v0.5.5 (Coder-side discipline §§4.11-4.16). Companion v0.5.6 ratified evening adding 8 non-blocking refinements per 3-Opus approval synthesis (Claude1+Claude2+Claude10 unanimous APPROVE-AS-IS). Three Phase 0.2 cycles documented: cycle-2 closed; cycle-3 dispatched on Common_utilities_API.md v0.3 (7 fabrications surfaced); cycle-4 dispatched on v0.4 (6 P0 + 9 P1 regressions surfaced); v0.5 produced incorporating all 15 fixes. NEW §4b prefilter pipeline section (`prepare_review.py` v1.0 → v1.3, regex → AST → 4-bug closure). §6 roster revised: 6 Geminis replaced with 3 GPTs (cycle-3+4 evidence: 6 hallucinated P0s, 0 unique findings). §8 items 25-29 added. Phase 0.2 active; cycle-5 dispatch pending on v0.5 with 12-reviewer panel; Phase 0.3 advisor pilot prep planned post cycle-5.

- **v0.5.4 (2026-05-03 afternoon, no Tech_SUMMARY bump — see QRC v0.5.4 changelog).** QRC §4.7 reframed to three-dimensional review assignment; §§4.8-4.10 new (2+2+1 deep section coverage, convergence handling, sprint-cap=5). No new MIWikiAI architectural model.

- **v0.5.3 (2026-05-03 morning, no Tech_SUMMARY bump — see QRC v0.5.3 changelog).** QRC §§2.7+4.5+4.6+4.7 new (prose-vs-VERBATIM consistency, dispatch-document required contents, pre-dispatch smoke test, six-aspect review taxonomy). Origin: cycle-3 dispatch-attempt evidence.

- **v0.5.2 (2026-05-02).** Two new MIWikiAI binding rules from cycle-1/cycle-2 experience. (a) MIWikiAI_QRC §3.5 reviewer source-access protocol — symmetric to Coder anti-silent-paraphrase rule; reviewer requests source if missing rather than producing finding without verification; cycle-1 Sonnet3-vs-Gemini2 contrast is the origin (Sonnet3 caught real bugs with source access; Gemini2 hallucinated without). (b) MIWikiAI_QRC §4.4 standardized review-bundle requirement — every `_API.md` cycle-2+ dispatch ships `<artifact>_review_bundle.zip` with MANIFEST + artifact + self-review + prior consolidated summary + sources + counter + governance. §8 §11 §12 updated. Cycle-3+ on `_API.md` artifacts is first compliance window (cycle-2 of Common_utilities_API.md v0.2 predates the bundle rule).
...
*Compiled by Claude9 (Claude Opus 4.7, MIWikiAI reviewer-ID Claude9, Coder role for this artifact) on 2026-05-05 (v0.5.5). Paired with `MIWikiAI_Quick_Reference_Card_v0_5_5.md`. Phase 0.2 status: cycle-2 closed; cycle-3 produced 7 fabrications surfaced + corrected; cycle-4 produced 6 P0 + 9 P1 surfaced + corrected in v0.5; cycle-5 dispatch pending on v0.5 (12-reviewer panel: 6 Sonnet + 3 GPT + 3 Opus). Prefilter v1.3 deployed. Awaits architect ratification.*

*No quota issues observed during compilation. Compiled per QRC v0.5.5 §4.16 against architect's real v0.5.4 (`/tmp/check_v05/Common_utilities_API_review_bundle/governance/MIWikiAI_Quick_Reference_Card_v0_5_4.md`, MD5-verified) and prior Tech_SUMMARY v0.5.2.*
