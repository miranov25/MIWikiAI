...
source_status: DRAFT v0.3 — direct source enumeration of Common/ at a pinned 40-character commit, replacing the Doxygen/search-derived evidence base of v0.1-v0.2. Closes the "full DIRECT directory listing NOT yet performed" gap that v0.2 declared. Six [VERIFY] flags resolved; three previously-unknown subdirectories found. Awaiting architect gate 4.
source_fingerprint:
  upstream:
...
known_verify_flags_closed_in_v0_3:
  ...
  - "[FIX-13] Third B-field polarity convention (Gemini1 A-1, deferred from v0.2 under architect ruling R-1) — CLOSED, finding REJECTED. MagneticField.h:L50-51 declares exactly kConvLHC and kConvDCS2008. See Appendix A.5."
known_verify_flags:
...
MagneticField* MagneticField::createFieldMap(float l3Cur,    // L3 current (kA)
                                             ...
                                             Int_t convention,  // kConvLHC or kConvDCS2008
                                             Bool_t uniform,
                                             float beamenergy,
...
**Polarity conventions.** Two active conventions:
...
### A.5 B-field polarity convention count — **resolves deferred FIX-13**

**Claim (v0.3).** Exactly **two** polarity conventions.

**Evidence.** [DIRECT] `Common/Field/include/Field/MagneticField.h`:L50-51:
...
**Result: CLOSED ✅ — Gemini1 A-1 REJECTED.** There is no third "Automatic/CCDB-managed" convention in the enum. FIX-13, deferred from v0.2 to v0.3 under architect ruling R-1, is hereby resolved **against** the proposed addition. The finding was `UNVERIFIED` when raised and does not survive direct source inspection.
...
| **kConvLHC** / **kConvDCS2008** | The two magnetic-field polarity conventions supported by `MagneticField::createFieldMap`. |
...
- **P1 — FIX-13 resolved against the finding.** Gemini1 A-1's proposed third "Automatic/CCDB-managed" B-field polarity convention **does not exist**. `MagneticField.h`:L50-51 has exactly `kConvLHC` and `kConvDCS2008`. Deferred from v0.2 under architect ruling R-1; now rejected on direct evidence.
